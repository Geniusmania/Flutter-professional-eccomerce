...
//register user
  Future<UserModel?> registerUser(String phone, String email, String password,
      String first_name, String last_name, String username) async {
  ...
  Future<UserModel?> login(String email, String password) async {
    final response = await http.post(Uri.parse('${Api.URL}/api/auth/login'),
        headers: {'Content-Type': 'Application/json; charset=UTF-8'},
        body: jsonEncode({'email': email, 'password': password}));
    final responseData = jsonDecode(response.body);
    if (response.statusCode == 200 || response.statusCode == 201) {
      final token =
          responseData['token']; // Correct key based on Postman response
